...
| Failure on empty input | crashes in `Brain.predict` (`policy[0]`) | All three endpoints declare `seed` as `@RequestParam(required = true)`, so a **missing** `seed` is rejected by Spring with `MissingServletRequestParameterException` → 400 before any service code runs. An **explicitly empty** `seed` (e.g. `?seed=`) reaches the service: `generate` rejects it (`IllegalArgumentException` → 400), `distribution` and `predict-next` accept it and return 200 with an empty distribution / null next token (only `seed == null` raises, and `?seed=` binds to `""`, not `null`). | `IllegalArgumentException` | `IllegalArgumentException` |
